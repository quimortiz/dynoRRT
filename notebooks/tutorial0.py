...
get_ipython().system('pip3 install meshcat # Only if you want to use Pinocchio for visualization')
...
print("Elapsed time [s]: ", toc - tic , " time per IK solutions" , (toc - tic) / len(ik_solutions))
print("out", out)
...
start =  np.array(
[ 1.51437234e-01 , 8.17017891e-01 ,-4.71108920e-01 ,-1.36055381e+00,
 -1.61660844e+00 , 7.37057021e-01 ,-4.84776655e-07]
...
ik_solutions = [ s  for s in ik_solutions if cm.is_collision_free(s) and np.sum( s < lb ) == 0 and np.sum( s > ub ) == 0 ]
